...
import (
	...
	"github.com/matchstalk/mss-core/tools/server/sets"
)
...
func (h *Handler) serveAggregated(resp http.ResponseWriter, req *http.Request) {
	...
	for checkName, check := range h.Checks {
		...
		if err := check(req); err != nil {
			log.Info("healthz check failed", "checker", checkName, "error", err)
	...
	// (directxman12): this should also accept a request for JSON content (via a accept header)
